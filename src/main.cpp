...
uint256 hashGenesisBlock("0x15af9edf9b33ed682c59a77a9a502282ecd6a211c6de79781f239bf8e82c94bb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Viptocoin: starting difficulty is 1 / 2^12
...
{
    // Viptocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 786 * COIN;

    // Subsidy is cut in half every 500k blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 500000); // Viptocoin: 500k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Viptocoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Viptocoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Viptocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Viptocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4c460b7bdf295d6dea27377788a17590548d06d627518531ad3f946f85e03d20");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Balqi casino - jxmiecole - viptocoin is the best crypto 21/09/21";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 786 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049fad83eaacd15ac3df3771c7194639e973ca80aa9448d0964cfa3509504e0aa1bfd1caecb82a3a514972ac2d52b3980e564297bce3c8406f7982a7566ab2657c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1632229322;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084845848;

        if (fTestNet)
        {
            block.nTime    = 1632229422;
            block.nNonce   = 386204453;
        }
       ...
       //// debug print
        ...
        assert(block.hashMerkleRoot == uint256("0xf604a7c1f42b61ce24dc2cc159162f65aec6da3ffd90db90fbeef3aa32a1266c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfl, 0xc2, 0xb8, 0xda }; // Viptocoin: randomly changed by human generation.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ViptocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ViptocoinMiner(CWallet *pwallet)
{
    printf("ViptocoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ViptocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ViptocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ViptocoinMiner, pwallet));
}
